...
![image](./images/masterIndex.png)
辅助索引的叶子节点的 data 域记录着主键的值，因此在使用辅助索引进行查找时，需要先查找到主键值，然后再到主索引中进行查找。

![image](./images/supoortIndex.png)
### 2. 哈希索引
...
非聚集索引叶节点仍然是索引节点，只是有一个指针指向对应的数据块，此如果使用非聚集索引查询，查询到记录的主键索引，再使用主键的值通过聚集索引查找到需要的数据
...
## mysql sql慢分析
线上开启slowlog，提取慢查询，然后仔细分析explain 中 type字段,key字段，以及extra字段，发生的具体场景及mysql是怎么做的